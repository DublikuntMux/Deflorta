...
function MakeURI() {
	...
	var filename = gDoc.path.substring(0, lastIndex) + ".xml";
...
function WriteReanimFrame(sampleFrame, frameArray, prev, URI, elementIndex) {
	...
	*/
	...
	if (prev.prevX != stringX) {
		gOutputBuffer += "<x>" + stringX + "</x>";
	}

	if (prev.prevY != stringY) {
		gOutputBuffer += "<y>" + stringY + "</y>";
	}

	if (prev.prevKX != stringDegreesKX) {
		gOutputBuffer += "<kx>" + stringDegreesKX + "</kx>";
	}

	if (prev.prevKY != stringDegreesKY) {
		gOutputBuffer += "<ky>" + stringDegreesKY + "</ky>";
	}

	if (prev.prevSX != stringSX) {
		gOutputBuffer += "<sx>" + stringSX + "</sx>";
	}

	if (prev.prevSY != stringSY) {
		gOutputBuffer += "<sy>" + stringSY + "</sy>";
	}

	if (prev.prevF != stringF) {
		gOutputBuffer += "<f>" + stringF + "</f>";
	}

	if (prev.prevA != stringA) {
		gOutputBuffer += "<a>" + stringA + "</a>";
	}

	if (prev.prevImageName != imageName) {
		gOutputBuffer += "<i>" + imageName + "</i>";
	}

	if (prev.prevFontName != fontName) {
		gOutputBuffer += "<font>" + fontName + "</font>";
	}

	if (prev.prevText != stringText) {
		if (stringText == "") {
			// underscore means blank string because the empty string means repeat previous
			gOutputBuffer += "<text>_</text>";
		}
		else {
			gOutputBuffer += "<text>" + XmlEncode(stringText) + "</text>";
		}
	}
...
function WriteReanimLayer(layerIndex, URI) {
	...
	for (elementIndex = 0; elementIndex < maxElements; elementIndex++) {
		...
		gOutputBuffer += "<name>" + layerName + "</name>\n";
...
function WriteReanimFile() {
	...
	if (!FLfile.write(URI, gOutputBuffer, "append")) {
		throw "Failed to write output buffer to file.";
	}
	FLfile.write(URI, "</reanim>", "append");
